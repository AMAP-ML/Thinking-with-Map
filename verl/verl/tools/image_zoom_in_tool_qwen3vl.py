...
class ImageZoomInTool(BaseTool):
    ...
    def _maybe_resize_bbox(self, bbox_2d: list[float], img_width: int, img_height: int) -> Optional[list[float]]:
        ...
        if height < self.MIN_DIMENSION or width < self.MIN_DIMENSION:
            ...
            if new_height > self.MIN_DIMENSION and new_width > self.MIN_DIMENSION:
            ...
            if new_height < self.MIN_DIMENSION or new_width < self.MIN_DIMENSION:
                logger.warning(
                    f"Final bbox size ({new_width}x{new_height}) are still smaller than minimum ({self.MIN_DIMENSION})."
                    f"Original bbox: {bbox_2d}, original image size: {img_width}x{img_height}"
                )
                return None
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs) -> tuple[ToolResponse, float, dict]:
        ...
        try:
            resized_bbox = self._maybe_resize_bbox(bbox_2d, img_width=image_width, img_height=image_height)

            if resized_bbox is None:
                error_msg = (
                    f"Error: The specified bounding box {bbox_2d} for original image {image_width}x{image_height} is invalid or results in a crop smaller than "
