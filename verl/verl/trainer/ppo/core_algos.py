...
from verl.trainer.ppo.PKPO import sloo_minus_one
...
@register_adv_est(AdvantageEstimator.PKPO)
def compute_passkpo_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    ...
    **kwargs,
    ...
    K = int(config.get("pkpo_k", kwargs.get("K", 4)))

    scores = token_level_rewards.sum(dim=-1)  # (bs,)
    advantages_scalar = torch.zeros_like(scores)

    id2indices = defaultdict(list)
    for i, gid in enumerate(index.tolist()):
    ...
    with torch.no_grad():
        for gid, idxs in id2indices.items():
            if len(idxs) < 2:
                raise ValueError(
                    f"Pass@k requires at least 2 samples per group. Got {len(idxs)} for group {gid}."
                )

            g = scores[idxs]
            group_vals = sloo_minus_one(g, K=K)  # (n,)

            if norm_adv_by_std_in_grpo:
                std = torch.std(g)
                group_vals = group_vals / (std + epsilon)

            advantages_scalar[idxs] = group_vals
