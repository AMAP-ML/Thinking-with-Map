...
def main():
    ...
    ALLOWED_DATA_SOURCES = {"hotpotqa", "2wikimultihopqa", "musique", "bamboogle"}
    logger.info(f"Filtering dataset to include only these data sources: {ALLOWED_DATA_SOURCES}")
    ...
    with tempfile.TemporaryDirectory() as tmp_download_dir:
        for split in ["train", "test"]:
            ...
            try:
                ...
                # )
                local_parquet_filepath = os.path.join(args.hf_repo_id, parquet_filename)
                ...
                # Filter the DataFrame to keep only rows with allowed data sources
                initial_row_count = len(df_raw)
                df_filtered = df_raw[df_raw["data_source"].isin(ALLOWED_DATA_SOURCES)].copy()
                filtered_row_count = len(df_filtered)
                logger.info(f"Filtered rows: {initial_row_count} -> {filtered_row_count}. Kept {filtered_row_count} rows.")

                # If no rows remain after filtering, skip to the next file
                if df_filtered.empty:
                    logger.warning(f"No rows remaining for split '{split}' after filtering. Skipping.")
                    continue
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hf_repo_id", default="data/nq_hotpotqa_train", help="HuggingFace dataset repository ID."
    )
    parser.add_argument(
        "--local_dir",
