...
if [ $USE_SGLANG -eq 1 ]; then
    pip install "sglang[all]==0.5.5" --no-cache-dir -i https://mirrors.tuna.tsinghua.edu.cn/pypi/web/simple && pip install torch-memory-saver --no-cache-dir -i https://mirrors.tuna.tsinghua.edu.cn/pypi/web/simple
fi
...
pip install "transformers[hf_xet]>=4.51.0" accelerate datasets peft hf-transfer \
    ...
    pytest py-spy pre-commit ruff tensorboard -i https://mirrors.tuna.tsinghua.edu.cn/pypi/web/simple
...
wget -nv https://github.com/Dao-AILab/flash-attention/releases/download/v2.8.1/flash_attn-2.8.1+cu12torch2.8cxx11abiFALSE-cp312-cp312-linux_x86_64.whl && \
    pip install --no-cache-dir flash_attn-2.8.1+cu12torch2.8cxx11abiFALSE-cp312-cp312-linux_x86_64.whl -i https://mirrors.tuna.tsinghua.edu.cn/pypi/web/simple


pip install --no-cache-dir flashinfer-python==0.3.1 -i https://mirrors.tuna.tsinghua.edu.cn/pypi/web/simple
...
if [ $USE_MEGATRON -eq 1 ]; then
    ...
    pip3 install --no-deps git+https://github.com/NVIDIA/Megatron-LM.git@core_v0.13.1 -i https://mirrors.tuna.tsinghua.edu.cn/pypi/web/simple
fi
...
pip install opencv-fixer -i https://mirrors.tuna.tsinghua.edu.cn/pypi/web/simple && \
